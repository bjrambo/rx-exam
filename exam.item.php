...
class examItem extends Object
{
	...
	function setAttribute($attribute)
	{
		...
		if($attribute->pass_group_list)
		{
			...
			{
				if($member_group_list[$pass_group_list[$i]])
				{
					$group_list[$pass_group_list[$i]] = $member_group_list[$pass_group_list[$i]]->title;
				}
			}
		}
		$this->adds($attribute);
	...
	{
		if(!$this->get('passGroupList'))
		{
			return false;
		}
		if(array_key_exists($key, $this->get('passGroupList')))
		{
			return true;
		}
	...
	function getQuestions()
	{
		...
		foreach($output->data as $key => $val)
		{
			...
			if($this->isGranted())
			{
				$accessible[$val->question_srl] = true;
			}
	...
	function getStatus()
	{
		if(!$this->get('status'))
		{
			return $this->getConfigStatus('', true);
		}
