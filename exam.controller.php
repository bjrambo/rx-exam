...
class examController extends exam
{
	...
	public function procExamCreate()
	{
		...
		if(!$args->exam_title)
		{
			return new Object(-1, 'msg_not_exam_title');
		}
		...
		if($this->module_info->exam_pass_group_option && $args->exam_pass_group_list)
		{
			...
			if($this->module_info->exam_pass_group_option > 1)
			{
				...
				if(!count($default_group_list))
				{
					unset($args->exam_pass_group_list);
				}
				for($i = 0; $i < count($args->exam_pass_group_list); $i++)
				{
					if(!in_array($args->exam_pass_group_list[$i], $default_group_list))
					{
						continue;
					}
			...
			else
			{
				...
				for($i = 0; $i < count($args->exam_pass_group_list); $i++)
				{
					if(!array_key_exists($args->exam_pass_group_list[$i], $default_group_list))
					{
						continue;
					}
					$group_list[] = $args->exam_pass_group_list[$i];
		...
		if($this->module_info->exam_pass_point_option && $args->exam_pass_point)
		{
			...
			if($pass_point_min > $args->exam_pass_point || $args->exam_pass_point > $pass_point_max)
			{
	...
	public function procExamQuestionInsert()
	{
		...
		if($params->q_type == 1)
		{
			if(!$params->q_answer6)
			{
				return new Object(-1, 'msg_not_answer');
			}
	...
	public function procExamQuestionDelete()
	{
		...
		// question_srl이 있으면 문제정보도 체크
		$questionitem = $oExamModel->getQuestion($question_srl);
		if(!$questionitem->isExists())
		{
			return new Object(-1, 'msg_not_founded 1');
		}
		if($questionitem->get('document_srl') != $examitem->document_srl)
		{
			return new Object(-1, 'msg_invalid_request');
		}
	...
	public function procExamJoin()
	{
		...
		{
			return new Object(-1, 'msg_exists_result');
		}

		// 시작시 생선한 세션이 존재하는지 체크
		if(!$_SESSION['exam_joinlog'][$examitem->document_srl])
		{
			return new Object(-1, 'msg_not_session');
		}

		// 혹시나.. 출제된 문제가 없는데 전송된경우 체크
		if(!$examitem->get('question_count'))
		{
			return new Object(-1, 'no_question_list');
		}
		...
		if($examitem->isDate() && ($examitem->get('start_date') || $examitem->get('end_date')))
		{
			...
			{
				return new Object(-1, 'msg_not_exam_date');
			}
		...
		if($examitem->isTime() && $examitem->get('exam_time'))
		{
			...
			if($over_time > 10)
			{
				return new Object(-1, 'msg_time_over');
			} // 처리시간으로 10초정도는 예외처리 둠
		}
	...
	function insertExam($obj, $manual_inserted = FALSE)
	{
		...
		if($obj->cutline > 100)
		{
			return new Object(-1, 'msg_invalid_exam_cutline');
		}
		...
		if($obj->is_date == "Y")
		{
			...
			{
				return new Object(-1, 'msg_invalid_start_time');
			}
		}
		...
		if($obj->is_time == "Y")
		{
			if(!$obj->exam_time)
			{
				return new Object(-1, 'msg_not_exam_time');
			}
		}
		unset($obj->regdate);
	...
	function updateExam($source_obj, $obj, $manual_updated = FALSE)
	{
		...
		if($obj->status)
		{
			$obj->status = $this->getConfigStatus($obj->status, true);
		}
	...
	function insertQuestion($obj)
	{
		...
		if(!$obj->question_srl)
		{
			$obj->question_srl = getNextSequence();
		}
	...
	{
		if(!$obj->question_srl || !$obj->document_srl)
		{
			return new Object(-1, 'msg_not_founded');
		}
	...
	{
		if(!$obj->log_srl || !$obj->document_srl)
		{
			return new Object(-1, 'msg_not_founded');
		}
	...
	function updateQuestionCount($document_srl, $question_count = 0)
	{
		...
		if(!$question_count)
		{
			$question_count = $oExamModel->getQuestionCount($document_srl);
		}
	...
	function updateCategoryCount($module_srl, $category_srl, $document_count = 0)
	{
		...
		if(!$document_count)
		{
			$document_count = $oExamModel->getCategoryExamCount($module_srl, $category_srl);
		}
